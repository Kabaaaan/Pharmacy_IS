...
class StatisticsPage(ctk.CTkFrame):
    ...
    def reset_filter(self):
        if self.entry_filter_date and self.entry_filter_date.winfo_exists():
    ...
    def load_orders_data(self, start_date: date = None):
        ...
        def fetch_data():
            try:
                ...
                self.after(100, lambda: self._handle_error(
        ...
        for order in self._orders_data:
            try:
                ...
                if isinstance(order_copy['date'], str):
                    order_copy['date'] = datetime.strptime(
                        order_copy['date'], '%Y-%m-%d')
    ...
    def _update_summary(self, orders):
        ...
        if self.lbl_orders and self.lbl_orders.winfo_exists():
            self.lbl_orders.configure(text=f"📦 {total_orders} заказов")
        if self.lbl_revenue and self.lbl_revenue.winfo_exists():
